...
export function updateProgressInNote(
  note: string,
  ...
  const noteWithoutProgress = note.replace(
    /<!-- BOOKMARK_PROGRESS\s*[\s\S]*?\s*-->\s*/g,
    ""
  );

  // Add new progress data at the beginning
  return (
    progressComment + (noteWithoutProgress ? "\n\n" + noteWithoutProgress : "")
  );
}

// Generate resume URL with timestamp
export function generateResumeUrl(
  originalUrl: string,
  timestamp: number,
  platform: string
): string {
  try {
    ...
    switch (platform) {
      ...
      case "x":
        // X.com (Twitter) uses ?t= parameter in seconds
        url.searchParams.set("t", timestamp.toString());
        break;
...
export function detectVideoPlatform(url: string): string {
  try {
    ...
    if (
      ...
      return "twitch";
    } else if (urlObj.hostname.includes("x.com")) {
      return "x";
    }
